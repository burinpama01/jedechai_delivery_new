...
หลักการ:
- 1 order ใช้คูปอง `platform_discount` หรือ `free_delivery` ได้อย่างใดอย่างหนึ่งเท่านั้น (เลือกใบที่ดีที่สุด)
- คูปอง `merchant_discount` สามารถใช้ได้ 1 ใบต่อออเดอร์เป็นค่าเริ่มต้น (แต่อาจเพิ่ม Config ให้ร้านค้าปรับเพดานจำนวนคูปองที่ยอมให้ซ้อนกันได้ในอนาคต)
...
|---|---|---|---:|---|
| `platform_discount` (ลด subtotal) | `merchant_discount` | `free_delivery`, `promo_code` (ถ้ากำหนดให้โค้ดไม่ซ้อน) | 1 | หักจาก subtotal เท่านั้น |
| `merchant_discount` (คูปองร้าน) | `platform_discount` หรือ `free_delivery` (อย่างใดอย่างหนึ่ง), `merchant_discount` (ถ้าตั้งค่าร้านอนุญาต) | `promo_code` (ถ้ากำหนดให้โค้ดไม่ซ้อน) | 1 (ค่าเริ่มต้น, ร้านปรับเพดานได้) | ใช้ซ้อนกันได้ถ้าตั้งค่าให้ทำได้ (เฉพาะร้านที่กำหนด) |
| `free_delivery` (ลดค่าส่ง) | `merchant_discount` | `platform_discount`, `promo_code` (ถ้ากำหนดให้โค้ดไม่ซ้อน) | 1 | cap ไม่เกินค่าส่งจริง |
...
เคส 4: stacking `platform_discount` กับ `free_delivery` (เลือกใช้อย่างใดอย่างหนึ่ง)
- subtotal 200, delivery_fee 30
- มีคูปอง: `platform_discount` 40 และ `free_delivery` 20
- ผลที่ต้องได้:
  - ใช้ร่วมกันไม่ได้ ผู้ใช้ต้องเลือกว่าจะใช้ลดค่าอาหาร 40 บาท หรือลดค่าส่ง 20 บาท
  - ถ้าระบบเลือกให้อัตโนมัติ ควรเลือก `platform_discount` 40 (เพราะประหยัดกว่า)
...
- Test cases:
  ...
  - Referral qualify timing: แจก reward หลัง referee “booking/ออเดอร์แรก completed” เท่านั้น (ไม่ใช่ pending/accepted)
  - Referral roles: ชวนข้าม role (customer->driver, driver->customer, driver->driver) ได้ และ reward ถูกประเภทตาม role
  - One-time dialogs (customer): มี dialog แจ้ง reward (referee/referrer + welcome_general) เด้งครั้งเดียว แล้ว mark read
  - One-time dialogs (driver): มี dialog แจ้ง reward wallet (referee/referrer) เด้งครั้งเดียว แล้ว mark read
  - Welcome coupon (non-referral): ลูกค้าใหม่ที่ไม่มาจาก referral ได้ welcome coupon หลัง completed ครั้งแรก และไม่แจกซ้ำ
  - Coupon usage recording: เมื่อ apply คูปองแล้ว มี record ใน coupon_usages ผูกกับ booking_id และ used_count ถูกต้อง
  - UI hide breakdown: WELCOME20/REFERRER20/REFFERER20 ต้อง “เห็นยอดส่วนลด” แต่ไม่เห็น code/ข้อความ breakdown ทั้งฝั่งลูกค้า/คนขับ/ร้าน
  - Notes leak: WELCOME20/REFERRER20/REFFERER20 ต้องไม่ใส่รายละเอียดคูปองลง notes (กันรั่วไปถึงคนขับ/ร้าน)
  - Funding (GP ระบบก่อน): ถ้าเป็น WELCOME20/REFERRER20/REFFERER20 ส่วนลดต้องหักจาก GP ระบบก่อน ถ้าไม่พอค่อยกระทบรายได้คนขับ
  - Funding edge case: ส่วนลดมากกว่า GP ระบบ → ต้องไม่ error และไม่เรียกหัก wallet ด้วยจำนวน 0
...
10) Requirement เฉพาะคูปอง WELCOME20 และ REFFERER20 (phase ถัดไป)
- WELCOME20 เป็นคูปองที่ “ลดสองฝั่ง” (ตัวอย่าง: ลดค่าส่ง 10 + ลดร้านค้า 10 รวม 20)
- REFFERER20 เป็นคูปองที่ “ลดสองฝั่ง” (ตัวอย่าง: ลดค่าส่ง 10 + ลดร้านค้า 10 รวม 20)
- การรับภาระส่วนลด (funding) ต้องพยายามหักจาก “GP ระบบ” ก่อน ถ้า GP ระบบไม่พอให้ค่อยกระจายไปที่คนขับ/ร้านค้าเพิ่ม เพื่อไม่ให้กระทบรายได้คนขับมาก
- หน้าลูกค้า/checkout **ไม่โชว์ breakdown** ว่าลดจากส่วนไหนบ้าง ให้ลูกค้าเห็นเพียงว่า “ใช้ส่วนลด/คูปองสำเร็จ” และยอดสุทธิที่ต้องจ่ายเท่านั้น รวมถึง **หน้าคนขับ** และ **หน้าร้านค้า** ไม่ควรเห็น breakdown

11) Referral input/reward per role (เพิ่มเติม - phase ถัดไป)
- role ร้านค้า: ไม่ต้องมีช่องกรอกโค้ดแนะนำ (ไม่มี flow ใช้งานคูปอง)
- role คนขับ: reward จาก referral ให้เป็น wallet (ไม่ใช่คูปอง)

12) Phase (ถัดไป): คนขับได้ wallet reward (ซิงค์ event เดียวกับลูกค้า)
- ใช้ event เดียวกันกับลูกค้า: แจก reward หลัง “booking/ออเดอร์แรก completed ของ referee”
- รองรับการชวนข้าม role และต้องซิงค์กัน:
  - ลูกค้าชวนเพื่อนมาเป็นคนขับ: ให้ wallet reward หลัง booking แรก completed ของ referee
  - คนขับชวนเพื่อนมาเป็นลูกค้า: ให้ wallet reward หลังออเดอร์แรก completed ของ referee
  - คนขับชวนเพื่อนมาเป็นคนขับ: ให้ wallet reward หลัง booking แรก completed ของ referee
- มูลค่า reward: wallet topup 100 บาท
- ต้องการให้เด้ง dialog ฝั่งคนขับ และให้แสดงที่หน้าจอ DriverDashboard
...
- default: ใช้ `platform_discount` หรือ `free_delivery` ได้ 1 ใบ และสามารถใช้ร่วมกับ `merchant_discount` ได้ 1 ใบ (ค่าเริ่มต้น)
- `promo_code`:
  - default: ไม่ซ้อนกับคูปองหลัก
- admin ปรับได้: ได้ (ผ่าน config)
*** platform_discount / free_delivery ไม่สามารถซ้อนกันได้ เลือกได้ 1 อย่าง
*** merchant_discount ค่าเริ่มต้นคือ 1 ใบ แต่สามารถให้ร้านค้าปรับเพดานการซ้อนคูปองของร้านตัวเองได้
...
- สรุป stacking matrix เวอร์ชัน final
  - **แบบที่ 1:** `platform_discount` (ส่วนลดค่าอาหารจากระบบ) + `merchant_discount` (ส่วนลดจากร้านค้า ค่าเริ่มต้น 1 ใบ หรือตามเพดานร้านกำหนด)
  - **แบบที่ 2:** `free_delivery` (ส่วนลดค่าส่ง) + `merchant_discount` (ส่วนลดจากร้านค้า ค่าเริ่มต้น 1 ใบ หรือตามเพดานร้านกำหนด)
  - **หมายเหตุ:** ไม่สามารถใช้ `platform_discount` คู่กับ `free_delivery` ได้ ต้องเลือกอย่างใดอย่างหนึ่ง
...
- นิยาม policy ชดเชยคนขับเมื่อส่วนลดค่าส่งเกิดขึ้น (source of fund + accounting)
  - `platform_discount`: GP ร้าน (ให้คนขับ 90% เข้า GP ระบบ 10%)
  - `free_delivery`: GP ร้าน (ให้คนขับ 90% เข้า GP ระบบ 10%)
  - `merchant_discount`: GP ร้าน (ให้คนขับ 90% เข้า GP ระบบ 10%)
  - `promo_code`: GP ร้าน (ให้คนขับ 90% เข้า GP ระบบ 10%)
...
- 2026-05-08: ผู้ใช้ยืนยันว่ารัน Supabase migrations ทั้งหมดครบแล้ว ให้ใช้ schema/RPC ล่าสุดเป็น baseline สำหรับงาน notification debugging
- 2026-05-08: พบ bug notification ล่าสุดที่ต้องแก้ต่อ: food order ใหม่ยังไม่ส่ง push ให้ร้านค้า, driver candidate สำหรับงานใหม่ถูก Edge Function authorization block เพราะยังไม่ใช่ participant, และ flow `ready_for_pickup` บางจุดไม่ trigger driver notification ซ้ำ
