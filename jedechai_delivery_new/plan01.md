...
- คูปอง `merchant_discount` สามารถใช้ซ้อนกันได้หลายใบ (ไม่จำกัดจำนวน หรืออาจจำกัดตาม config)
...
| `merchant_discount` (คูปองร้าน) | `platform_discount` หรือ `free_delivery` (อย่างใดอย่างหนึ่ง), `merchant_discount` (ใบอื่น) | `promo_code` (ถ้ากำหนดให้โค้ดไม่ซ้อน) | ไม่จำกัด (หลายใบ) | ใช้ซ้อนกันได้หลายใบ (เฉพาะร้านที่กำหนด) |
...
- default: ใช้ `platform_discount` หรือ `free_delivery` ได้ 1 ใบ และสามารถใช้ร่วมกับ `merchant_discount` ได้หลายใบซ้อนกัน
- `promo_code`:
...
*** merchant_discount สามารถซ้อนกันได้ เนื่องจากเป็นของร้านค้า
...
- สรุป stacking matrix เวอร์ชัน final
  - **แบบที่ 1:** `platform_discount` (ส่วนลดค่าอาหารจากระบบ) + `merchant_discount` (ส่วนลดจากร้านค้า สามารถใช้ได้หลายใบซ้อนกันได้)
  - **แบบที่ 2:** `free_delivery` (ส่วนลดค่าส่ง) + `merchant_discount` (ส่วนลดจากร้านค้า สามารถใช้ได้หลายใบซ้อนกันได้)
