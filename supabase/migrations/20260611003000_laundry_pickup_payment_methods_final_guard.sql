-- Final laundry pickup/payment guard.
-- Must replay after laundry core so pickup payment columns and RPC overrides are
-- applied only after public.laundry_orders/public.bookings exist.

ALTER TABLE public.laundry_orders
  ADD COLUMN IF NOT EXISTS pickup_presence text DEFAULT 'remote_pickup';

UPDATE public.laundry_orders
SET pickup_presence = 'remote_pickup'
WHERE pickup_presence IS NULL;

ALTER TABLE public.laundry_orders
  ALTER COLUMN pickup_presence SET DEFAULT 'remote_pickup',
  ALTER COLUMN pickup_presence SET NOT NULL;
...
GRANT EXECUTE ON FUNCTION public.driver_confirm_laundry_pickup(uuid, text) TO authenticated, service_role;
