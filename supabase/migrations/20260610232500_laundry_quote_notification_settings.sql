...
ALTER TABLE IF EXISTS public.profiles
  ADD COLUMN IF NOT EXISTS laundry_quote_sound_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS laundry_quote_sound_key text NOT NULL DEFAULT 'merchant_new_laundry_quote';

ALTER TABLE IF EXISTS public.profiles
  DROP CONSTRAINT IF EXISTS profiles_laundry_quote_sound_key_check,
  ADD CONSTRAINT profiles_laundry_quote_sound_key_check
    CHECK (char_length(btrim(COALESCE(laundry_quote_sound_key, ''))) BETWEEN 1 AND 80);
...
DECLARE
  ...
  v_sound_key text := 'merchant_new_laundry_quote';
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(BTRIM(p.laundry_quote_sound_key), ''), 'merchant_new_laundry_quote')
