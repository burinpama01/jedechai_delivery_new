-- Add server-side date validation to apply_coupon_atomic.
-- Prevents clients with manipulated device clocks from bypassing coupon expiry.
...
begin
  ...
  -- Server-side date range check using now() — immune to device clock manipulation
